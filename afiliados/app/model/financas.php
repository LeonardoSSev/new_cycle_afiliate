<?php
    Class Financas{
		...
		public function pegaSaldob($pdo,$userid){
		
            $sql = "SELECT saldo FROM saldobloqueado where userid = :userid order by id DESC";
		...
		public function ComprarVoucher($pdo,$userid){
			...
			$sqlb = "INSERT INTO pagamentos (userid,valor,descricao,posicao,datavencimento,registro) VALUES (:userid,:valor,'Compra de Voucher','1',:datav,:datas) ";
			$objb = $pdo->prepare($sqlb);
			$objb->bindParam(":userid",$userid);
			$objb->bindParam(":valor",$valor);
			$objb->bindParam(":datas",$datas);
			$objb->bindParam(":datav",$datavencimento);
			$objb->execute();
		...
		public function ComprarVoucherb($pdo,$userid){
			...
			$sql = "SELECT * FROM configuracoes WHERE id = '16' ";
			$obj = $pdo->prepare($sql);
			$obj->execute();
			$linha = $obj->fetch(PDO::FETCH_ASSOC);
			$valor = $linha['valor'];
	
			$sqlb = "INSERT INTO pagamentos (userid,valor,descricao,posicao,datavencimento,registro) VALUES (:userid,:valor,'Compra de Voucher','4',:datav,:datas) ";
			$objb = $pdo->prepare($sqlb);
			$objb->bindParam(":userid",$userid);
			$objb->bindParam(":valor",$valor);
			$objb->bindParam(":datas",$datas);
		...
		public function AtivarVoucher($pdo,$userid){
			...
			if($contagem >= '1'){
				...
				$sqlb = "UPDATE voucher SET ativacao='1' WHERE userid = :userid AND saque='1'";
				$objb = $pdo->prepare($sqlb);
				$objb->bindParam(":userid",$userid);
				$objb->execute();
				
				$sqlc = "UPDATE acesso SET saque='1' WHERE userid = :userid";
				$objc = $pdo->prepare($sqlc);
				$objc->bindParam(":userid",$userid);
				$objc->execute();
		...
		public function AtivarVoucherb($pdo,$userid){
			...
			if($contagem >= '1'){
				...
				$sqlb = "UPDATE voucher SET ativacao='1' WHERE userid = :userid AND fase='1'";
				$objb = $pdo->prepare($sqlb);
				$objb->bindParam(":userid",$userid);
				$objb->execute();
			...
			$sqlc = "SELECT * FROM indicados WHERE userid = :userid AND fase='1' ORDER BY id ASC";
			$objc = $pdo->prepare($sqlc);
			$objc->bindParam(":userid",$userid);
			$objc->execute();
			...
			$sqld = "UPDATE indicados SET fase1 = '1' WHERE userid = :userid AND fase='1' ORDER BY id ASC";
			$objd = $pdo->prepare($sqld);
			$objd->bindParam(":userid",$userid);
			$objd->execute();
			
			$upgrade = $ciclos->Upgrade($pdo,2,$cupom,$userid);
