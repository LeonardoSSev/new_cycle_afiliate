...
<html>
    ...
    <body style='background-color: #fafafa; color: black !important'>
        <div style='width:700px; background: #ffffff; border: 1px solid #dddddd; border-radius: 3px; padding: 20px;margin-left: auto; margin-right: auto; '>
            <div style='text-align: center; font-size:33px;'>Ola, <div style='font-weight: bolder;'>$nome</div></div>
            
            <div style='text-align: center; padding:20px;margin-top:15px;'>
                <img src='https://cdeanewcycle.com.br/afiliado/images/logosb.png' width='150' >
            </div>
          
            <div style='padding:20px 0 25px;text-align:center;color:#ffffff'>
            <a href='https://cdeanewcycle.com.br/afiliado/recuperar?i=$bash' style='color:#ffffff;text-decoration:none;display:inline-block;text-align:center;background:#4183c4;background-color:#4183c4;border-radius:5px;padding:12px 44px;font-weight:bold;letter-spacing:normal;font-size:17px; margin:0 auto;width:auto!important' target='_blank'>RECUPERAR SENHA</a>
            </div>
...
";
        ...
        $headers .= 'From: <noreply@cdeanewcycle.com.br>' . "\r\n";
	...
	public function listaUsuarios($pdo){
		...
		$sql = "SELECT acesso.userid,acesso.ativacao,acesso.bloqueio,acesso.usuario,dadospessoais.nome,dadospessoais.email,dadospessoais.whatsapp FROM acesso,dadospessoais WHERE acesso.userid = dadospessoais.userid";
		$obj = $pdo->prepare($sql);
		$obj->execute();
	...
	public function pegaDadosNewpay($pdo, $userid){
	...
	public function pegaDadosBankon($pdo, $userid){
	...
	public function pegaDadosPicpay($pdo, $userid){
	...
	public function pegaAdmVerDadosBancarios($pdo, $id){
		...
		$sql = "SELECT * FROM dadosbancarios WHERE  userid = :id ";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":id",$id);
		$obj->execute();
	...
	public function adicionarDadosBancariosPicpay($pdo, $banco,$picpay,  $userid){
		$datas = date("Y-m-d H:i:s");
		
		$sql = "INSERT into dadosbancarios (banco,picpay, userid,registro) values (:banco,:picpay,:userid,'".$datas."')";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":banco",$banco);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":picpay",$picpay);
		$obj->execute();
		
		return ($obj) ? $obj : false;
	}

	public function adicionarDadosBancariosLitecoin($pdo, $banco,$litecoin,  $userid){
		$datas = date("Y-m-d H:i:s");
		
		$sql = "INSERT into dadosbancarios (banco,litecoin, userid,registro) values (:banco,:litecoin,:userid,'".$datas."')";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":banco",$banco);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":litecoin",$litecoin);
		$obj->execute();
		
		return ($obj) ? $obj : false;
	}

	public function adicionarDadosBancariosEthereum($pdo, $banco,$ethereum, $userid){
		$datas = date("Y-m-d H:i:s");
		
		$sql = "INSERT into dadosbancarios (banco,ethereum, userid,registro) values (:banco,:ethereum,:userid,'".$datas."')";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":banco",$banco);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":ethereum",$ethereum);
		$obj->execute();
		
		return ($obj) ? $obj : false;
	}

	public function adicionarDadosBancarios($pdo,$banco, $agencia, $conta, $tipoconta, $operacao, $nometitular, $cpftitular,  $userid){
		$datas = date("Y-m-d H:i:s");
		
		$sql = "INSERT into dadosbancarios (banco,agencia,conta,tipoconta,cpftitular,nometitular,operacao, userid,registro) values (:banco,:agencia,:conta,:tipoconta,:cpftitular,:nometitular,:operacao,:userid,'".$datas."')";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":banco",$banco);
	...
	public function editarDadosBancariosPicpay($pdo, $picpay,$id, $userid){

		
		$sql = "UPDATE dadosbancarios SET picpay = :picpay  WHERE userid = :userid AND id = :id";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":id",$id);
		$obj->bindParam(":picpay",$picpay);
	...
	public function editarDadosBancariosLitecoin($pdo, $litecoin,$id, $userid){
		
		$sql = "UPDATE dadosbancarios SET litecoin = :litecoin  WHERE userid = :userid AND id = :id";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":id",$id);
		$obj->bindParam(":litecoin",$litecoin);
	...
	public function editarDadosBancariosEthereum($pdo, $ethereum,$id, $userid){
		
		$sql = "UPDATE dadosbancarios SET ethereum = :ethereum  WHERE userid = :userid AND id = :id";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":id",$id);
		$obj->bindParam(":ethereum",$ethereum);
	...
	public function editarDadosBancarios($pdo,  $banco, $agencia, $conta, $tipoconta, $operacao, $nometitular, $cpftitular, $id, $userid){
		
		$sql = "UPDATE dadosbancarios SET banco = :banco, agencia = :agencia, conta = :conta, tipoconta = :tipoconta, operacao = :operacao, nometitular = :nometitular, cpftitular = :cpftitular  WHERE userid = :userid AND id = :id";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":id",$id);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":banco",$banco);
	...
	public function AdmeditarDadosBancarios($pdo, $urpay,  $i){
		
		$sql = "UPDATE dadosbancarios SET urpay = :urpay  WHERE userid = :id";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":id",$i);
		$obj->bindParam(":urpay",$urpay);
		$obj->execute();
...
";
	...
	public function bloqueioUsuarios($pdo,$userid){

		$sql = "UPDATE acesso SET bloqueio = '1' WHERE userid = :userid";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
	...
	public function desbloqueioUsuarios($pdo,$userid){

		$sql = "UPDATE acesso SET bloqueio = '0' WHERE userid = :userid";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
	...
	public function pegaVoucher($pdo,$userid){

		$sql = "SELECT * FROM voucher WHERE userid = :userid AND ativacao='0' AND saque='1'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);

		return $linha['voucher'];
	}
	
	
	public function pegaVoucherb($pdo,$userid){
		
		// jogar todos para saque='1'

		$sql = "SELECT * FROM voucher WHERE userid = :userid AND ativacao='0' AND fase='1'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);
	...
	public function pegaFase($pdo,$userid){
	...
	public function pegaCiclo($pdo,$userid, $nivel){
		
		// jogar todos para saque='1'

		$sql = "SELECT * FROM indicados WHERE userid = :userid AND fase = :nivel";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":nivel",$nivel);
		$obj->execute();
	...
	public function Recaptcha($pdo){
		
		$sql = "SELECT motivo,chaveapi FROM configuracoes WHERE id = '18'";
        $obj = $pdo->prepare($sql);
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);
	...
	public function FechaCiclo($pdo,$userid, $nivel){
		
		// jogar todos para saque='1'

		$sql = "SELECT * FROM indicados WHERE userid = :userid AND fase = :nivel AND fase$nivel = '1'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->bindParam(":nivel",$nivel);
		$obj->execute();
	...
	public function pegaHabilitar($pdo,$userid){
		
		$sql = "SELECT * FROM indicados WHERE userid = :userid AND habilitar='1'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$contagem = $obj->rowCount();
	...
	public function PegaRede($pdo,$userid){
		...
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);
		$ativacao = $linha['ativacao'];

		return $ativacao;
	}

	public function PegaExpirado($pdo,$userid){
		
		$sql = "SELECT * FROM indicados WHERE userid = :userid AND ativacao > '1'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$contagem = $obj->rowCount();
	...
	public function PegaReentrada($pdo,$userid){
		
		$sql = "SELECT * FROM acesso WHERE userid = :userid AND reentrada = '1'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$contagem = $obj->rowCount();
	...
	public function pegaDadosBancariosh($pdo,$userid){
	...
	public function Esperar($pdo,$userid){
		
		$sql = "SELECT * FROM pagamentos WHERE userid = :userid AND esperar = '1' AND ativacao='0'";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$contagem = $obj->rowCount();
	...
	public function UltimoAtivo($pdo,$userid){
		
		$sql = "SELECT * FROM acesso WHERE sponsorid = :userid AND ativacao='1' ORDER BY id DESC";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);
		...
		$sqlb = "SELECT * FROM pagamentos WHERE userid = :userid AND ativacao='1' AND fase='1' ORDER BY id ASC";
		$objb = $pdo->prepare($sqlb);
		$objb->bindParam(":userid",$iduser);
		$objb->execute();
		$linhab = $objb->fetch(PDO::FETCH_ASSOC);
		$dataativacao = $linhab['dataativacao'];
	...
	public function UltimoCadastro($pdo,$userid){
		
		$sql = "SELECT * FROM acesso WHERE sponsorid = :userid  ORDER BY id DESC";
		$obj = $pdo->prepare($sql);
		$obj->bindParam(":userid",$userid);
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);
	...
	public function QtdeReentrada($pdo,$userid){
		...
		$obj->execute();
		$linha = $obj->fetch(PDO::FETCH_ASSOC);
		$qtdereentrada = $linha['qtdereentradas'];

	
		return $qtdereentrada;
	}
	
	public function Termos($pdo,$userid){
